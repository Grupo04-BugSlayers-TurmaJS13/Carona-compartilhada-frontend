...
//     destino: "Niterói",
//     valor: 15,
//     veiculo: { modelo: "Civic", cor_veiculo: "Prata" },
//     usuario: { nome: "Carlos Mendes" },
//     status: "ativo",
//     data: "2026-04-22",
//   },
//   {
//     id: 2,
//     embarque: "Barra da Tijuca",
//     destino: "Centro RJ",
//     valor: 20,
//     veiculo: { modelo: "Corolla", cor_veiculo: "Branco" },
...
//     destino: "São Paulo",
//     valor: 85,
//     veiculo: { modelo: "HB20", cor_veiculo: "Preto" },
//     usuario: { nome: "Rafael Lima" },
//     status: "ativo",
//     data: "2026-04-22",
//   },
//   {
...
//     destino: "Recreio",
//     valor: 25,
//     veiculo: { modelo: "Onix", cor_veiculo: "Azul" },
//     usuario: { nome: "Julia Costa" },
//     status: "ativo",
//     data: "2026-04-24",
//   },
// ];
...
export default function Viagens() {
  ...
  async function buscarViagens() {
    try {
      ...
      await buscar("/listarviagens", setViagens, {
  ...
  const filtradas = viagens.filter(
    (v) =>
      ...
      (!applied || !filterData || v.dataAgendamento === filterData),
  );
        ...
        <div className="min-h-screen w-full bg-[var(--color-background)] text-[var(--color-foreground)] overflow-x-hidden flex flex-col items-center">
          ...
          <main className="relative z-10 w-full max-w-6xl flex flex-col items-center px-4 sm:px-6 md:px-12 pt-24 md:pt-32 pb-20 md:pb-24">
            <section className="w-full flex flex-col items-center text-center mb-20">
              ...
              <h1
                className="w-full font-black text-[var(--color-foreground-white)] mb-6 tracking-tighter leading-[0.9] text-center text-3xl sm:text-4xl md:text-5xl" >
                Para onde vamos <br />
                <span className="text-[var(--color-primary-light)]">hoje?</span>
              </h1>

              <p
                className="text-[var(--color-foreground-muted)] mb-12 font-medium max-w-md mx-auto"
              ...
              <div className="w-full max-w-5xl bg-[var(--color-background-card)] border border-[var(--color-stroke)] rounded-2xl md:rounded-full p-3 md:p-2 md:pl-6 shadow-2xl flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-2">
                ...
                <div className="flex flex-1 items-center gap-3 w-full">
                  ...
                  <input
                    placeholder="Partida"
                    className="bg-transparent w-full outline-none text-[var(--color-foreground-high)] placeholder:text-[var(--color-foreground-low)] font-bold text-sm md:text-sm"
                    value={embarque}
                ...
                <div className="flex flex-1 items-center gap-3 w-full">
                  <SlidersHorizontal
                    size={16}
                    className="text-[var(--color-foreground-low)] shrink-0"
                  />
                  <input
                    placeholder="Filtrar destino..."
                    className="bg-transparent w-full outline-none text-[var(--color-foreground-high)] placeholder:text-[var(--color-foreground-low)] font-semibold text-sm md:text-sm"
                    value={filterDestino}
                    onChange={(e) => {
                ...
                <div className="flex flex-[0.7] items-center gap-3 w-full">
                  ...
                  <input
                    type="date"
                    className="bg-transparent w-full outline-none text-[var(--color-foreground-high)] font-semibold [color-scheme:dark] cursor-pointer text-sm md:text-sm"
                    value={filterData}
                    onChange={(e) => {
            ...
            <section className="w-full mb-24">
              <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 pb-8 border-b border-[var(--color-stroke)] gap-4">
                <div>
                  <h2
                    className="font-black text-[var(--color-foreground-white)] tracking-tight mb-1"
                    style={{ fontSize: "var(--text-xl)" }}
                  >
                    Caronas Disponíveis
                  </h2>
                  <p className="text-[var(--color-foreground-low)] text-sm font-medium">
                    {/* {ativos} motorista{ativos !== 1 ? "s" : ""} ativo
                {ativos !== 1 ? "s" : ""} agora */}
                  </p>
                </div>
              ...
              {viagens.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 text-center w-full">
                  <div className="h-20 w-20 rounded-3xl bg-[var(--color-background-subtle)] border border-[var(--color-stroke)] flex items-center justify-center mb-6">
                    <Search
                      size={32}
                      className="text-[var(--color-foreground-low)]"
                    />
                  </div>
            ...
            <section className="w-full flex flex-col items-center text-center rounded-[3rem] border border-[var(--color-stroke)] bg-gradient-to-b from-[var(--color-background-card)] to-transparent p-8 md:p-16">
              ...
              <h3
                className="font-black text-[var(--color-foreground-white)] tracking-tight mb-4"
                style={{ fontSize: "var(--text-xl)" }}
              >
                Rentabilize seu trajeto
              </h3>
              <p
                className="text-[var(--color-foreground-muted)] mb-10 font-medium leading-relaxed max-w-md mx-auto"
