...
import {
  Search, MapPin, Navigation, Plus, Car,
  Sparkles, SlidersHorizontal, Calendar, X,
} from "lucide-react";
...
export default function Viagens() {
  ...
  const buscarViagens = useCallback(async () => {
    try {
      setIsLoading(true);
      await buscar("/viagens", setViagens, { headers: { Authorization: token } });
    } catch (error: any) {
      if (error.toString().includes("401")) {
        handleLogout();
      } else {
        ToastAlerta("Erro ao carregar viagens.", "erro");
      }
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (token) buscarViagens();
  }, [buscarViagens]);

  const filtradas = viagens.filter((v) => {
    const matchEmbarque = v.embarque.toLowerCase().includes(embarque.toLowerCase());
    const matchDestino = v.destino.toLowerCase().includes(destino.toLowerCase());
    const matchFilterDestino = !applied || !filterDestino ||
      v.destino.toLowerCase().includes(filterDestino.toLowerCase());
    const matchData = !applied || !filterData ||
      (v.dataAgendamento && String(v.dataAgendamento).slice(0, 10) === filterData);
    return matchEmbarque && matchDestino && matchFilterDestino && matchData;
  });
  ...
  const temFiltroAtivo = embarque || destino || filterDestino || filterData;
  ...
  return (
    <PageShell>
      <div className="min-h-screen w-full bg-[var(--color-background)] text-[var(--color-foreground)] overflow-x-hidden flex flex-col items-center">
        ...
        <main className="relative z-10 w-full max-w-6xl flex flex-col items-center px-4 sm:px-6 md:px-12 pt-24 md:pt-32 pb-20 md:pb-24">
          <section className="w-full flex flex-col items-center text-center mb-20">
            ...
            <h1 className="w-full font-black text-[var(--color-foreground-white)] mb-6 tracking-tighter leading-[0.9] text-center text-3xl sm:text-4xl md:text-5xl">
              Para onde vamos <br />
            ...
            <div className="w-full max-w-5xl bg-[var(--color-background-card)] border border-[var(--color-stroke)] rounded-2xl md:rounded-full p-3 md:p-2 md:pl-6 shadow-2xl flex flex-col md:flex-row items-stretch md:items-center gap-3 md:gap-2">
              <div className="flex flex-1 items-center gap-3 w-full">
                ...
                <input
                  ...
                  value={embarque}
                  onChange={(e) => { setEmbarque(e.target.value); setApplied(false); }}
                />
              </div>
              ...
              <div className="flex flex-1 items-center gap-3 w-full">
                ...
                <input
                  ...
                  className="bg-transparent w-full outline-none text-[var(--color-foreground-high)] placeholder:text-[var(--color-foreground-low)] font-bold text-sm"
                  value={destino}
                  onChange={(e) => { setDestino(e.target.value); setApplied(false); }}
                />
              </div>
              ...
              <div className="flex flex-1 items-center gap-3 w-full">
                ...
                <input
                  ...
                  className="bg-transparent w-full outline-none text-[var(--color-foreground-high)] placeholder:text-[var(--color-foreground-low)] font-semibold text-sm"
                  value={filterDestino}
              ...
              <div className="flex flex-[0.7] items-center gap-3 w-full">
                <Calendar size={20} className="text-[var(--color-foreground-low)] shrink-0" />
                <input
                  type="date"
                  className="bg-transparent w-full outline-none text-[var(--color-foreground-high)] font-semibold [color-scheme:dark] cursor-pointer text-sm"
                  value={filterData}
              ...
              <div className="flex items-center gap-2 w-full md:w-auto">
                ...
                <button
                  ...
                  className="w-full md:w-auto py-4 md:py-3.5 flex items-center justify-center gap-2 px-8 rounded-full bg-[var(--color-primary)] hover:bg-[var(--color-primary-light)] active:scale-95 text-black font-black text-sm transition-all shadow-lg"
                >
          ...
          <section className="w-full mb-24">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 pb-8 border-b border-[var(--color-stroke)] gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                {temFiltroAtivo && (
                  <button
                    onClick={handleClear}
                    className="inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl border border-[var(--color-stroke)] bg-[var(--color-background-card)] text-[var(--color-foreground-muted)] text-xs font-semibold hover:border-red-600/40 hover:text-red-400 transition-all"
                  >
                    <X size={14} />
                    Limpar filtros
                  </button>
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-24 text-center w-full">
                ...
                <p className="font-bold text-[var(--color-foreground-muted)] text-lg mb-2">
                  Nenhuma carona encontrada
                </p>
                <p className="text-sm text-[var(--color-foreground-low)]">
          ...
          <section className="w-full flex flex-col items-center text-center rounded-[3rem] border border-[var(--color-stroke)] bg-gradient-to-b from-[var(--color-background-card)] to-transparent p-8 md:p-16">
