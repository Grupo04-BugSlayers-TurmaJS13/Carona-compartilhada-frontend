...
import { buscar, contratar } from "../../../services/Service"
import type Viagem from "../../../models/Viagem"
import { ToastAlerta } from "../../../utils/ToastAlerta"
import { PageShell } from "../../about/AboutShared"

function ContratarViagem() {
    ...
    async function contratarViagem() {
        setIsLoading(true)
        try {
            await contratar(`/viagens/contratar/${id}`,
                { usuarioContratanteId: usuario.id },
                { headers: { Authorization: token } }
            )
            ToastAlerta("Viagem contratada com sucesso!", "sucesso")
            navigate("/viagens")
        } catch (error: any) {
            if (error.toString().includes("401")) handleLogout()
            else ToastAlerta("Erro ao contratar viagem!", "erro")
        } finally {
            setIsLoading(false)
        }
    }
                ...
                <div className="min-h-screen flex items-center justify-center bg-(--color-background) px-4">
                    <div className="w-full max-w-md bg-(--color-background-card) border border-(--color-stroke) rounded-2xl p-6 shadow-(--shadow-soft) text-center">
                        ...
                        <p className="text-(--color-foreground-muted) mb-6">
                            Tem certeza que deseja contratar a viagem:
                        </p>
                        ...
                        <div className="flex gap-8 items-center justify-center ">
                            ...
                            <div className=" w-full flex justify-center items-center border border-green-600 rounded-full">
                                <button
                                    onClick={contratarViagem}
                                    disabled={isLoading}
                                    className="w-1/2 py-3 rounded-lg font-bold bg-(--color-success) text-white hover:bg-green-600 transition"
                                >
...
export default ContratarViagem