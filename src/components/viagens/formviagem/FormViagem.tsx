...
const viagemInicial: Viagem = {
  ...
  usuario: { id: 0, nome: "", usuario: "", senha: "", foto: "", viagens: [] },
};
...
function FormViagem() {
  ...
  async function buscarViagemPorId(viagemId: string) {
    await buscar(
      ...
      (data: unknown) => {
        ...
        setViagem({
          ...raw,
          tempo_vigagem: raw.tempoViagem ?? 0,
          formaPagamento: raw.pagamento ?? "Dinheiro",
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(async () => {
      try {
        ...
        if (!isCancelled) {
          setRouteFeedback(
            "Digite um embarque e destino válidos para calcular automaticamente.",
          );
          ToastAlerta(
  ...
  useEffect(() => {
    const valorCalculado = Number(
      (viagem.distancia * 1 + viagem.tempo_vigagem * 0.2).toFixed(2),
    );
    if (Number.isNaN(valorCalculado)) return;
    setViagem((current) =>
  ...
  async function gerarNovaViagem(e: SyntheticEvent<HTMLFormElement>) {
    ...
    const payloadBase = {
      ...resto,
      tempoViagem: Number(tempo_vigagem),
      pagamento: formaPagamento,
      dataAgendamento: dataAgendamento !== "" ? dataAgendamento : null,
      dataEncerramento: dataEncerramento !== "" ? dataEncerramento : null,
      veiculo: { id: viagem.veiculo.id },
      usuario: { id: viagem.usuario.id },
    };

    const payload =
      id !== undefined ? { ...payloadBase, id: Number(id) } : payloadBase;

    try {
      if (id !== undefined) {
        await atualizar("/viagens", payload, setViagem, {
          headers: { Authorization: token },
        });
        ToastAlerta("A viagem foi atualizada com sucesso!", "sucesso");
      } else {
        await cadastrar("/viagens", payload, setViagem, {
          headers: { Authorization: token },
        });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  step="0.1"
                  readOnly
                  className="p-3 rounded-lg bg-(--color-background-subtle) border border-(--color-stroke) text-white opacity-90"
