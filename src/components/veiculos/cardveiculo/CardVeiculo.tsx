        ...
        <div className="bg-[var(--color-background-card)] border border-[var(--color-stroke)] rounded-2xl overflow-hidden shadow-[var(--shadow-soft)] hover:scale-[1.02]
            ...
            <div className="relative h-52 overflow-hidden">
                ...
                <span className="absolute top-3 left-3 bg-[var(--color-primary)]
                        text-black rf-xs font-semibold px-3 py-1 rounded-full shadow-[var(--shadow-bip)]">
                    {veiculo.tipo}
                </span>
            </div>
            ...
            <div className="p-4 flex flex-col gap-2">
                ...
                <h2 className="rf-lg font-semibold text-[var(--color-primary)]">
                    {veiculo.marca} {veiculo.modelo}
                </h2>

                <div className="rf-sm text-[var(--color-foreground-muted)] space-y-1">
                ...
                {showActions && (
                    <div className="flex gap-3">
                        <Link to={`/atualizarveiculo/${veiculo.id}`} className="w-full py-2 flex items-center rounded-full justify-center font-medium rf-sm
                                    text-[var(--color-foreground-high)] hover:border hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition">
                            Editar
                        </Link>

                        <Link to={`/deletarveiculo/${veiculo.id}`} className="w-full py-2 flex items-center justify-center font-medium rf-sm hover:border rounded-full hover:border-red-600
